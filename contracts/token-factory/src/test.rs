...
// Vote snapshot tests
#[test]
fn test_vote_weight_strategy_serialization() {
    let env = Env::default();
    
    let one_vote = VoteWeightStrategy::OneAddressOneVote;
    let token_balance = VoteWeightStrategy::TokenBalance;
    
    let one_vote_bytes = env.to_storage_val(&one_vote);
    let token_balance_bytes = env.to_storage_val(&token_balance);
    
    let one_vote_restored: VoteWeightStrategy = env.from_storage_val(&one_vote_bytes);
    let token_balance_restored: VoteWeightStrategy = env.from_storage_val(&token_balance_bytes);
    
    assert_eq!(one_vote_restored, VoteWeightStrategy::OneAddressOneVote);
    assert_eq!(token_balance_restored, VoteWeightStrategy::TokenBalance);
}

#[test]
fn test_vote_snapshot_serialization() {
    let env = Env::default();
    ...
    let snapshot = VoteSnapshot {
        ...
        proposal_id: 42,
        timestamp: 1000,
        strategy: VoteWeightStrategy::TokenBalance,
        total_weight: 1_000_000,
    };
    
    let bytes = env.to_storage_val(&snapshot);
    let restored: VoteSnapshot = env.from_storage_val(&bytes);
    
    assert_eq!(restored.id, 1);
    assert_eq!(restored.proposal_id, 42);
    assert_eq!(restored.timestamp, 1000);
    assert_eq!(restored.strategy, VoteWeightStrategy::TokenBalance);
    assert_eq!(restored.total_weight, 1_000_000);
}

#[test]
fn test_voter_weight_serialization() {
    let env = Env::default();
    
    let voter = Address::generate(&env);
    let voter_weight = VoterWeight {
        snapshot_id: 1,
        voter: voter.clone(),
        weight: 500,
    };
    
    let bytes = env.to_storage_val(&voter_weight);
    let restored: VoterWeight = env.from_storage_val(&bytes);
    
    assert_eq!(restored.snapshot_id, 1);
    assert_eq!(restored.voter, voter);
    assert_eq!(restored.weight, 500);
}

#[test]
fn test_governance_proposal_with_snapshot() {
    let env = Env::default();
    
    let proposer = Address::generate(&env);
    let payload_hash = soroban_sdk::BytesN::from_array(&env, &[5u8; 32]);
    ...
    assert_eq!(restored.snapshot_id, 123);
    assert_eq!(restored.id, 1);
}

#[test]
fn test_vote_weight_immutability_simulation() {
    let env = Env::default();
    
    // Simulate snapshot at proposal start
    let voter1 = Address::generate(&env);
    let voter2 = Address::generate(&env);
    
    // Initial balances at snapshot time
    let snapshot = VoteSnapshot {
        id: 1,
        proposal_id: 1,
        timestamp: 1000,
    ...
    let weight1 = VoterWeight {
        snapshot_id: 1,
        voter: voter1.clone(),
        weight: 1000,
    };
    
    let weight2 = VoterWeight {
        snapshot_id: 1,
        voter: voter2.clone(),
        weight: 500,
    };
    
    // Serialize snapshot and weights
    let snapshot_bytes = env.to_storage_val(&snapshot);
    let weight1_bytes = env.to_storage_val(&weight1);
    let weight2_bytes = env.to_storage_val(&weight2);
    
    // Simulate balance changes after snapshot (should not affect voting power)
    // In real implementation, votes would use stored weights, not current balances
    
    // Restore and verify weights remain unchanged
    let restored_weight1: VoterWeight = env.from_storage_val(&weight1_bytes);
    let restored_weight2: VoterWeight = env.from_storage_val(&weight2_bytes);
    
    assert_eq!(restored_weight1.weight, 1000);
    assert_eq!(restored_weight2.weight, 500);
    
    // Verify total weight is preserved
    let restored_snapshot: VoteSnapshot = env.from_storage_val(&snapshot_bytes);
    assert_eq!(restored_snapshot.total_weight, 1500);
}

#[test]
fn test_one_address_one_vote_strategy() {
    let env = Env::default();
    ...
    // In OneAddressOneVote, all voters have weight = 1
    let snapshot = VoteSnapshot {
        id: 1,
        proposal_id: 1,
        timestamp: 1000,
        strategy: VoteWeightStrategy::OneAddressOneVote,
        total_weight: 3, // 3 voters
    };
    ...
    assert_eq!(weight1.weight, 1);
    assert_eq!(weight2.weight, 1);
    assert_eq!(weight3.weight, 1);
    assert_eq!(snapshot.total_weight, 3);
}

#[test]
fn test_token_balance_strategy() {
    let env = Env::default();
    
    let whale = Address::generate(&env);
    let small_holder = Address::generate(&env);
    
    // TokenBalance strategy: weight = token balance at snapshot
    let snapshot = VoteSnapshot {
        id: 1,
        proposal_id: 1,
        timestamp: 1000,
    ...
    let whale_weight = VoterWeight {
        snapshot_id: 1,
        voter: whale,
        weight: 10_000,
    };
    
    let small_weight = VoterWeight {
        snapshot_id: 1,
        voter: small_holder,
        weight: 500,
    };
    ...
    assert_eq!(whale_weight.weight, 10_000);
    assert_eq!(small_weight.weight, 500);
    assert_eq!(snapshot.total_weight, 10_500);
}

#[test]
fn test_snapshot_prevents_manipulation() {
    let env = Env::default();
    
    let manipulator = Address::generate(&env);
    
    // Snapshot taken at proposal start (t=1000)
    ...
    let initial_weight = VoterWeight {
        snapshot_id: 1,
        voter: manipulator.clone(),
        weight: 100, // Balance at snapshot time
    };
    
    // Serialize at snapshot time
    let weight_bytes = env.to_storage_val(&initial_weight);
    
    // Simulate manipulator acquiring more tokens after snapshot
    // (In real scenario, balance would change but voting power wouldn't)
    
    // Restore weight - should still be original snapshot value
    let restored_weight: VoterWeight = env.from_storage_val(&weight_bytes);
    
    // Voting power remains at snapshot value, not current balance
    assert_eq!(restored_weight.weight, 100);
    assert_eq!(restored_weight.snapshot_id, 1);
}
